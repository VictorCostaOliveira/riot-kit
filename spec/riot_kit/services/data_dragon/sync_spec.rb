...
RSpec.describe RiotKit::Services::DataDragon::Sync do
  ...
  it "grava JSONs e retorna versão com sucesso" do
    ...
    allow(client).to receive(:get_versions).and_return(HttpResponses.ok(["14.1.1"]))
    allow(client).to receive(:get_items).and_return(
      HttpResponses.ok(
        data: {
          "3078" => {
            name: "IE",
    ...
    allow(client).to receive(:get_champions).and_return(
      HttpResponses.ok(
        data: {
    ...
    allow(client).to receive(:get_summoner_spells).and_return(
      HttpResponses.ok(
        data: {
    ...
    allow(client).to receive(:get_runes).and_return(
      HttpResponses.ok(
        [
          {
            slots: [
